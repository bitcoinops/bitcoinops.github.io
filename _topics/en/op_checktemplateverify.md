...
excerpt: >
  **OP_CHECKTEMPLATEVERIFY (CTV)** is a proposed new opcode that takes a
  ...
  the opcode to include a set of outputs that match the commitment.  This
...
- **Congestion control transactions:** a spender pays a single output
  ...
  paid prior the follow-up transaction being confirmed.
...
- **Joinpools:** similar to a channel factory, multiple users can [pool
  ...
  onchain version of this requires generating and transmitting O(n!)
  offchain signatures for _n_ parties, making it impractical for more
  than a few users to participate in any individual joinpool with
  per-party exit paths.  Using CTV, each signature operation can be
  replaced with a CTV operation, which leaves the computational
  ...
  to do any way to validate the script they're paying).  Again, only the
  one round of signatures for each state transition would need to be
  communicated.  Alternative consensus change proposal specifically
...
- **Exploding keys:** this [proposal][dryja exploding keys] provides the
  ...
  digest.  Exploding keys can, in theory, save a few vbytes compared to
  CTV---although it also loses some of CTV's flexibility.
...
- **Makes script harder to analyze:** CTV tracks information about the
  executed opcode that precedes it as part of its protection against
  recursive covenants.  This is abnormal among opcodes
  (`OP_CODESEPARATOR` in its interaction with `OP_CHECKSIG` and variants
  being the only current opcode we're aware of that requires similar
  tracking).  At least one reviewer argues that dealing with the
  abnormal state requirements of CTV [adds][oconnor complexity] "much
  more complexity to anyone trying to formally model Script semantics".
...
- **CTV and OP_CHECKSIGFROMSTACK:** the [OP_CHECKSIGFROMSTACK][topic
  ...
  [LN-Symmetry][topic eltoo] (eltoo) and [Ark][topic ark]-style
  [joinpools][topic joinpools].

## History

CTV was originally proposed as `OP_CHECKOUTPUTSHASHVERIFY` (COSHV) and
was briefly renamed to `OP_SECURETHEBAG` before becoming
...
[update][ctv extensions] to the proposal allowed use of 20-byte hash
digests in addition the existing allowed use of 32-byte hash digests,
plus allowed the inclusion of additional commitments that must be

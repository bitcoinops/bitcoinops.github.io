...
value below a _target_ value that is the dynamically set by the
...
For example, image the ratio between the primary target and secondary
...
paid for 999 blocks out of every 1,000 she generates, reducing her
...
downside of the way GetBlock and Stratum v1 are typically used is that
pool members have no insight or direct control what they mine.  They
aren't directly informed about which transactions are included in the
template block and the pool may reject any attempt they make to change
...
allow individual pool members to chose which transactions to include in
...
interfering with its member's choices about what transactions to mine.
BraidPool is inspired by the former [P2Pool][] decentralized mining pool
but attempts mitigate some of P2Pool's problems, including its

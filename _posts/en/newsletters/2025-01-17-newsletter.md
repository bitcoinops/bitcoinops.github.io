...
- **Continued discussion about rewarding pool miners with ecash:**
  ...
  pool would implement the extra code and accounting to handle ecash
  ...
  pool about 10 BTC.  However, if the pool does mine that block and,
  ...
  including using various [proxies for actual fees][news304 fpps proxy].
  ...
  it earns in order to finance whatever payout scheme it chooses to pay
  its individual miners.  Once the intermediate pool acquired a
...
- **Offchain DLCs:** developer conduition [posted][conduition offchain]
  to the DLC-dev mailing list about a contract protocol that allows a
